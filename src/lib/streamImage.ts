...
export async function streamImage(
  ...
  if (!res.ok || !res.body) {
    throw new Error(`Не удалось создать кадр: ${res.status} ${await res.text().catch(() => "")}`);
  }
  ...
  if (!sawAnyEvent) {
    ...
    if (!replay.ok) {
      throw new Error(`Не удалось создать кадр: ${replay.status}`);
    }
    const json = (await replay.json()) as { data?: { b64_json?: string }[] };
    const b64 = json.data?.[0]?.b64_json;
    if (!b64) throw new Error("Кадр не был создан");
    onFrame(`data:image/png;base64,${b64}`, true);
    return;
  }
  if (!sawCompleted) throw new Error("Создание кадра прервалось");
}
